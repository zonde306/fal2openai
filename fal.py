...
async def format_messages(
    ...
    processed.pop(0)
    for prompt in reversed(processed):
        if not full:
...
async def send_message(
    messages: list[dict], api_key: str, model: str, reasoning: bool = False
) -> AsyncGenerator[dict, None]:
    client = (
        CLIENTS.get(api_key, fal_client.AsyncClient(api_key))
        if api_key
        else next_client()
    )
    ...
    try:
        assert (
            len(prompt1) < defines.PROMPT_CHARS_LIMIT
            and len(prompt2) < defines.PROMPT_CHARS_LIMIT
        ), f"Prompt too long: {len(prompt1)} + {len(prompt2)}"
